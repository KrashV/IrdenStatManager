...
--get unicode codepoint from utf8 char at byte index i
function utf8.codepoint(s, i)
    ...
    elseif c >= 0xE0 and c <= 0xEF then
        ...
        return (c - 0xE0) * 0x1000 + (c2 - 0x80) * 0x40 + (c3 - 0x80)
    elseif c >= 0xF0 and c <= 0xF4 then
        local c2 = s:byte(i + 1)
        local c3 = s:byte(i + 2)
        local c4 = s:byte(i + 3)
        ...
        return (c - 0xF0) * 0x40000 + (c2 - 0x80) * 0x1000 + (c3 - 0x80) * 0x40 + (c4 - 0x80)
    end
    return nil
end

--convert unicode codepoint to utf8 string
function utf8.char(cp)
    ...
    elseif cp <= 0x10FFFF then
        local c1 = 0xF0 + math.floor(cp / 0x40000)
        local c2 = 0x80 + (math.floor(cp / 0x1000) % 0x40)
        local c3 = 0x80 + (math.floor(cp / 0x40) % 0x40)
        local c4 = 0x80 + (cp % 0x40)
        return string.char(c1, c2, c3, c4)
    end
    return '�'
end

--mapping tables for cyrillic and latin characters
local upper_map = {
    ...
    y = 'Y', z = 'Z',
    --cyrillic lowercase -> uppercase (russian)
...
local lower_map = {
    ...
    Q = 'q', R = 'r', S = 's', T = 't', U = 'u', V = 'v', W = 'w', X = 'x',
    Y = 'y', Z = 'z',
    --cyrillic uppercase -> lowercase (russian)
    ['А'] = 'а', ['Б'] = 'б', ['В'] = 'в', ['Г'] = 'г', ['Д'] = 'д', ['Е'] = 'е',
    ['Ё'] = 'ё', ['Ж'] = 'ж', ['З'] = 'з', ['И'] = 'и', ['Й'] = 'й', ['К'] = 'к',
    ['Л'] = 'л', ['М'] = 'м', ['Н'] = 'н', ['О'] = 'о', ['П'] = 'п', ['Р'] = 'р',
    ['С'] = 'с', ['Т'] = 'т', ['У'] = 'у', ['Ф'] = 'ф', ['Х'] = 'х', ['Ц'] = 'ц',
...
--convert string to uppercase (supports latin and cyrillic)
function utf8.upper(s)
    local result = {}
    local i = 1
    while i <= #s do
        local nexti = utf8.next(s, i) or #s + 1
        local char = s:sub(i, nexti - 1)
        local upper_char = upper_map[char]
        table.insert(result, upper_char or char)
        i = nexti
    end
    return table.concat(result)
end

--convert string to lowercase (supports latin and cyrillic)
function utf8.lower(s)
    ...
    while i <= #s do
        local nexti = utf8.next(s, i) or #s + 1
        local char = s:sub(i, nexti - 1)
        local lower_char = lower_map[char]
        table.insert(result, lower_char or char)
        i = nexti
    end
    return table.concat(result)
end
